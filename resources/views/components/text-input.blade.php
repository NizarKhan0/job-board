    ...
    @if ('textarea' !== $type)
        ...
        <input x-ref="input-{{ $name }}" type="{{ $type }}" placeholder="{{ $placeholder }}"
            ...
            @class([
                ...
                'border-red-500' => $errors->has($name), // Red border if there's an error
                'ring-green-300' => !$errors->has($name) && old($name) !== null,
                'border-green-500' => !$errors->has($name) && old($name) !== null, // Green border if no error
            ]) />
    @else
...
{{-- yang bawah ni takde input h=green kalau betul, dia tnjuk error red je --}}
    ...
    @if ('textarea' !== $type)
        ...
        <input x-ref="input-{{ $name }}" type="{{ $type }}" placeholder="{{ $placeholder }}"
            ...
            @class([
                ...
                'ring-slate-300' => !$errors->has($name),
                'ring-red-300' => $errors->has($name),
                'border-red-500' => $errors->has($name), // Ensure the border color is also changed
            ]) />
    @else
        <textarea id="{{ $name }}" name="{{ $name }}" @class([
            'w-full rounded-md border-0 py-1.5 px-2.5 text-sm ring-1 placeholder:text-slate-400 focus:ring-2',
